...
###**두 범주로 나눠서, 두 변수 모두 유지하기. (한번도 안 해본 그룹과, 나머지로.)
...
#17. PRXYDATA
df17<-df15%>%filter(PRXYDATA %in% c(1))
table(df15$PRXYDATA) #무시..할까..? 아니면 yes만 남길까?
#18. 감정변수들
#a<-df17%>%filter((YODPREV %in% c(1,2))&(YOWRHRS %in% c(1,2,3,4))&(YOLOSEV %in% c(1,2))&(YODSCEV %in% c(1,2)))
table(df17$YODPREV)
df18<-df17%>%filter(YODPREV %in% c(1,2))
table(df17$YOWRHRS)
table(df17$YOLOSEV)
table(df17$YODSCEV)

write.csv(df18,"final_df3.csv")
